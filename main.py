...
By doing this, we don't have to load model again and again for every request.
Model will be saved in memory.

async - the function will not execute immediatlly but task written in function will be saved in memory 
and executed later
yield -Python internal event loop so that other tasks can run concurrently while functions waits for I/O operation
"""
dl_model = {} #{1. BiGRU, 2. Tokenizer}-> True , {} -> False

@asynccontextmanager # do 2 tasks in same function
